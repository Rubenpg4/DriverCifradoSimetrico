...
- [Lógica Interna (Core Logic)](#-lógica-interna-core-logic)
...
## ⚙ Lógica Interna (Core Logic)

### Algoritmo de Generación de Claves

La función `generate_aes_key()` utiliza el generador criptográficamente seguro del kernel:
...
for (int i = 0; i < AES_KEY_SIZE; i++) {
    ...
    get_random_bytes(&rand_val, sizeof(rand_val));  // CSPRNG del kernel
...
sequenceDiagram
    ...
    participant D as Device0
    participant K as Kernel CSPRNG
    participant B as keyAES Buffer
    
    U->>D: read()
    D->>K: get_random_bytes() x32
    K-->>D: 32 valores aleatorios
    loop Para cada byte
        D->>D: Mapear a allowed_chars
    end
    D->>B: Almacenar clave
    D-->>U: return 0
```
...
El padding PKCS#7 asegura que los datos sean múltiplo del tamaño de bloque AES (16 bytes):

```mermaid
graph LR
    subgraph "Entrada: 'Hola' (4 bytes)"
        A1[H] --> A2[o] --> A3[l] --> A4[a]
    end
    
    subgraph "Padding: 12 bytes (valor 0x0C)"
        P1["0x0C"] --> P2["0x0C"] --> P3["0x0C"] --> P4["..."] --> P12["0x0C"]
    end
    
    subgraph "Salida: 16 bytes (1 bloque AES)"
        O1[H] --> O2[o] --> O3[l] --> O4[a] --> O5["0x0C"] --> O6["..."] --> O16["0x0C"]
    end
```
...
// Siempre añade 1-16 bytes de padding
```
...
sequenceDiagram
    ...
    participant P as PKCS7 Padding
    participant C as Crypto API
    participant H as Hex Encoder
    participant B as encrypted_data
    
    U->>D1: write("Mensaje secreto")
    D1->>P: apply_pkcs7_padding()
    P-->>D1: datos paddeados (múltiplo de 16)
    D1->>C: crypto_alloc_skcipher("ecb(aes)")
    D1->>C: crypto_skcipher_setkey(keyAES, 32)
    D1->>C: crypto_skcipher_encrypt()
    C-->>D1: ciphertext binario
    D1->>H: bin_to_hex()
    H-->>D1: ciphertext hexadecimal
    D1->>B: almacenar resultado
    D1-->>U: return bytes_escritos
```
...
sequenceDiagram
    ...
    participant H as Hex Decoder
    participant C as Crypto API
    participant B as decrypted_data
    
    U->>D2: write("4a8f2c3d...")
    D2->>H: my_hex_to_bin()
    H-->>D2: ciphertext binario
    D2->>C: crypto_alloc_skcipher("ecb(aes)")
    D2->>C: crypto_skcipher_setkey(keyAES, 32)
    D2->>C: crypto_skcipher_decrypt()
    C-->>D2: plaintext (con padding)
    D2->>B: almacenar resultado
    D2-->>U: return bytes_escritos
```
...
```mermaid
stateDiagram-v2
    [*] --> Unloaded: Sistema arranca
    
    Unloaded --> Loading: insmod DriverCifradoDescifrado.ko
    Loading --> Initializing: init_driver()
    
    Initializing --> Registering: alloc_chrdev_region()
    Registering --> ClassCreating: class_create()
    ClassCreating --> DeviceCreating: device_create() x3
    DeviceCreating --> Ready: Éxito
    
    Ready --> Processing: open() desde userspace
    Processing --> Ready: release()
    
    Ready --> Unloading: rmmod
    Unloading --> Cleanup: exit_driver()
    Cleanup --> Unloaded: Recursos liberados
    
    Initializing --> ErrorState: Fallo
    Registering --> ErrorState: Fallo
    ClassCreating --> ErrorState: Fallo
    DeviceCreating --> ErrorState: Fallo
    ErrorState --> Unloaded: Cleanup parcial
```
...
```mermaid
graph TD
    START([Inicio]) --> GEN{¿Clave existente?}
    
    GEN -->|No| GENKEY["cat /dev/.../0<br/>Genera clave aleatoria"]
    GEN -->|Sí, usar propia| SETKEY["echo -n 'clave32bytes' > /dev/.../0"]
    
    GENKEY --> ENCRYPT
    SETKEY --> ENCRYPT
    
    ENCRYPT["echo -n 'mensaje' > /dev/.../1<br/>Cifrar mensaje"]
    ENCRYPT --> READENC["cat /dev/.../1<br/>Obtener hex cifrado"]
    
    READENC --> STORE["Almacenar/transmitir<br/>texto cifrado"]
    
    STORE --> RECEIVE["Recibir texto<br/>cifrado"]
    
    RECEIVE --> DECRYPT["echo -n 'hex' > /dev/.../2<br/>Descifrar"]
    
    DECRYPT --> READDEC["cat /dev/.../2<br/>Obtener mensaje original"]
    
    READDEC --> END([Fin])
    
    style START fill:#9f9
    style END fill:#f99
    style ENCRYPT fill:#99f
    style DECRYPT fill:#ff9
```
